...
import time
import pandas as pd
from matplotlib.backends.backend_svg import FigureCanvasSVG  # 用于处理 SVG 图像
# 设置随机种子
np.random.seed(42)
# 读取数据并进行预处理
...
# 构建模型
base_model = Xception(weights=None, input_shape=(256, 256, 3))
x = base_model.output
...
plt.title("Confusion Matrix - Xception_original")
plt.savefig("confusion_matrix_Xception_original.png")
plt.close()
...
combined_df.to_csv('Xception_original.csv', index=False)
